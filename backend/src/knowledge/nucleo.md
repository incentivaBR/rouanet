# BASE LEGAL DOS MECANISMOS DE INCENTIVO
<!-- fonte: frontend/biblioteca-juridica.html -->

F03 · Biblioteca Jurídica
## A lei está do seu lado.
O laudo também.
Base normativa completa dos 7 mecanismos de destinação do IR — leis, instruções normativas, fichas DIRPF e requisitos de comprovante. Para orientar clientes com segurança jurídica total.
Ver fichas por modalidade →
Espaço do Contador
mecanismos com base legal consolidada
anos de consolidação normativa (desde 1991)
risco na malha fina com limites e documentação corretos
84%
dos contribuintes do DF desconhecem (CRC-DF 2021)
BASE NORMATIVA POR MODALIDADE
### 7 Fichas Legais Completas
Selecione a modalidade para ver lei, IN RFB, ficha DIRPF e requisitos do comprovante.
🎭 Rouanet
⚽ Esporte
👧 FDCA
👴 FDI
🏥 PRONON
💊 PRONAS
♻️ Reciclagem
#### Lei Rouanet — Art. 18
Atividade Cultural e Artística — destinação direta ao projeto aprovado via SALIC
Lei nº 8.313/1991
6% do IR Devido
DIRPF: Incentivos Fiscais → PRONAC/Cultura
Base Legal
Lei nº 8.313, de 23 de dezembro de 1991, Art. 18 — Programa Nacional de Apoio à Cultura (PRONAC). Destinação direta ao projeto cultural aprovado pelo Ministério da Cultura via Sistema de Apoio às Leis de Incentivo à Cultura (SALIC).
Norma RFB
Instrução Normativa RFB nº 1.131/2011 e atualizações. Consulte sempre a IN vigente no exercício da declaração.
Limite
6% do IR Devido — independente dos demais mecanismos. O valor destinado abate diretamente do IR Devido. Custo líquido zero ao final do ano-calendário.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo (deduções legais). Exige imposto a pagar antes da destinação.
Ficha DIRPF
Deduções → Incentivos Fiscais → PRONAC/Cultura Art. 26 (patrocínio, 80%) fica em ficha diferente — confirme sempre qual modalidade o projeto usa antes de orientar.
Comprovante
Recibo de Mecenato emitido pelo proponente contendo: CNPJ do proponente, número do PRONAC, valor destinado, data da transferência, nome e CPF do destinador. Guardar também o comprovante bancário (PIX/TED).
Como verificar
Validar PRONAC ativo em salic.cultura.gov.br — projetos com captação encerrada ou suspensa não são elegíveis para novas destinações.
⚠️ Art. 18 vs Art. 26: Art. 18 = 100% abatível do IR Devido (destinação direta). Art. 26 = 80% abatível (patrocínio). São tratamentos distintos na DIRPF. Confirme qual consta no PRONAC do projeto antes de orientar o cliente.
#### Lei de Incentivo ao Esporte
Projetos desportivos e paradesportivos aprovados pelo Ministério do Esporte
Lei nº 11.438/2006
7% do IR Devido
DIRPF: Incentivos Fiscais → Desporto
Base Legal
Lei nº 11.438, de 29 de dezembro de 2006 — incentivo a projetos desportivos e paradesportivos aprovados e habilitados pelo Ministério do Esporte.
Norma RFB
Instrução Normativa RFB nº 1.339/2013 e atualizações. Consulte a IN vigente no exercício da declaração.
Limite
7% do IR Devido — o maior limite individual entre os mecanismos. Independente dos demais. O valor abate diretamente do IR Devido.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Deduções → Incentivos Fiscais → Desporto
Comprovante
Recibo emitido pelo projeto contendo: CNPJ do projeto esportivo, número de aprovação ministerial, valor destinado e data da transferência. Guardar também o comprovante bancário.
Como verificar
Confirmar aprovação ativa no portal do Ministério do Esporte. O projeto deve estar com captação em andamento no período da destinação.
#### FDCA — Fundo da Criança e do Adolescente
Estatuto da Criança e do Adolescente — destinação ao fundo municipal, estadual ou federal
Lei nº 8.069/1990 — ECA, Art. 260 II
6% do IR Devido
DIRPF: Doações Efetuadas → Código 40
Base Legal
Lei nº 8.069, de 13 de julho de 1990 (Estatuto da Criança e do Adolescente — ECA), Art. 260, inciso II. Destinação direta ao Fundo Municipal, Estadual ou Nacional dos Direitos da Criança e do Adolescente (FDCA / FIA).
Norma RFB
Instrução Normativa RFB vigente para o exercício. O fundo deve estar devidamente registrado no CNPJ e com conta corrente ativa junto ao CDCA competente.
Limite
6% do IR Devido — limite independente do FDI. Os dois mecanismos (FDCA + FDI) têm limites separados: cada um até 6%, sem limite conjunto entre eles.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Doações Efetuadas → Código 40
Comprovante
Recibo emitido pelo fundo contendo: CNPJ do fundo, número do recibo, valor destinado e data da transferência. O comprovante bancário (PIX/TED) é indispensável.
Como verificar
Confirmar conta corrente e CNPJ do FDCA diretamente com o CDCA (Conselho dos Direitos da Criança e do Adolescente) antes de transferir.
⚠️ Atenção: Confirme a conta bancária do FDCA-DF diretamente com o CDCA antes de realizar a transferência. Contas desatualizadas são o erro mais frequente nesse mecanismo.
#### FDI — Fundo do Idoso
Política Nacional do Idoso — destinação ao fundo municipal, estadual ou federal
Lei nº 12.213/2010
6% do IR Devido
DIRPF: Doações Efetuadas → Código 41
Base Legal
Lei nº 12.213, de 20 de janeiro de 2010 — institui o Fundo Nacional do Idoso e autoriza a destinação de parcela do IR ao Fundo Nacional, Estadual ou Municipal do Idoso.
Norma RFB
Instrução Normativa RFB vigente para o exercício. O fundo deve estar devidamente registrado com CNPJ e conta corrente ativa.
Limite
6% do IR Devido — limite independente do FDCA. Os dois mecanismos (FDCA + FDI) têm limites separados: cada um até 6%, sem limite conjunto entre eles.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Doações Efetuadas → Código 41
Comprovante
Recibo emitido pelo fundo contendo: CNPJ do fundo, número do recibo, valor destinado e data da transferência. Guardar também o comprovante bancário.
#### PRONON — Apoio à Atenção Oncológica
Destinação a entidade habilitada de oncologia pelo Ministério da Saúde
Lei nº 12.715/2012, Art. 1º
até 1% do IR Devido*
DIRPF: Incentivos Fiscais → PRONON
Base Legal
Lei nº 12.715, de 17 de setembro de 2012, Art. 1º — Programa Nacional de Apoio à Atenção Oncológica (PRONON). Destinação a entidades de saúde habilitadas pelo Ministério da Saúde para atendimento em oncologia.
Norma RFB
Instrução Normativa RFB vigente para o exercício. A entidade deve estar habilitada pelo Ministério da Saúde e constar no CNES (Cadastro Nacional de Estabelecimentos de Saúde).
Limite
Até 1% do IR Devido — este é o limite conjunto entre PRONON e PRONAS somados . Os dois mecanismos compartilham esse teto de 1%.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Deduções → Incentivos Fiscais → PRONON
Comprovante
Recibo emitido pela entidade habilitada contendo: CNPJ da entidade, número CNES, valor destinado e data da transferência. Guardar também o comprovante bancário.
⚠️ Limite conjunto: PRONON e PRONAS somados não podem ultrapassar 1% do IR Devido. Se o cliente usar ambos, a soma das duas destinações deve respeitar esse teto de 1%.
#### PRONAS — Atenção à Pessoa com Deficiência
Destinação a entidade habilitada de atenção à pessoa com deficiência
Lei nº 12.715/2012, Art. 3º
até 1% do IR Devido*
DIRPF: Incentivos Fiscais → PRONAS
Base Legal
Lei nº 12.715, de 17 de setembro de 2012, Art. 3º — Programa Nacional de Apoio à Atenção da Saúde da Pessoa com Deficiência (PRONAS/PCD). Destinação a entidades habilitadas pelo Ministério da Saúde.
Norma RFB
Instrução Normativa RFB vigente para o exercício. A entidade deve estar habilitada pelo Ministério da Saúde e constar no CNES.
Limite
Até 1% do IR Devido — este é o limite conjunto entre PRONON e PRONAS somados . Os dois mecanismos compartilham esse teto de 1%.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Deduções → Incentivos Fiscais → PRONAS
Comprovante
Recibo emitido pela entidade habilitada contendo: CNPJ da entidade, número CNES, valor destinado e data da transferência. Guardar também o comprovante bancário.
⚠️ Limite conjunto: PRONON e PRONAS somados não podem ultrapassar 1% do IR Devido. Se o cliente usar ambos, a soma das duas destinações deve respeitar esse teto de 1%.
#### Recicla+ — Lei de Incentivo à Reciclagem
Destinação a cooperativas de catadores habilitadas pelo MAPA
Lei nº 14.260/2021
6% do IR Devido
DIRPF: Incentivos Fiscais → Reciclagem
Base Legal
Lei nº 14.260, de 8 de dezembro de 2021 — institui incentivo fiscal para pessoas físicas que apoiam cooperativas e associações de catadores de materiais recicláveis habilitadas pelo Ministério da Agricultura, Pecuária e Abastecimento (MAPA).
Norma RFB
Instrução Normativa RFB vigente para o exercício. Lei relativamente recente (2021) — confirme sempre na IN do exercício corrente se há atualizações regulatórias.
Limite
6% do IR Devido — independente dos demais mecanismos.
Quem pode usar
Pessoa Física com IR Devido positivo, declaração obrigatória no modelo completo .
Ficha DIRPF
Deduções → Incentivos Fiscais → Reciclagem
Comprovante
Recibo emitido pela cooperativa contendo: CNPJ da cooperativa, número do recibo, valor destinado e data da transferência. Confirmar que a habilitação MAPA está ativa.
Como verificar
Verificar habilitação ativa da cooperativa junto ao MAPA antes de realizar a transferência.
⚠️ Mecanismo mais recente (2021): Confirme com a cooperativa que a habilitação MAPA está vigente e que a entidade emite recibo com todos os dados exigidos pela RFB. Menos habilitadas disponíveis que nas modalidades mais antigas.
VISÃO CONSOLIDADA
### Tabela de Limites — 7 Mecanismos
Mecanismo
Base Legal
Limite PF
Ficha DIRPF
🎭 Rouanet Art. 18
Lei 8.313/1991
Incentivos Fiscais → PRONAC/Cultura
⚽ Lei do Esporte
Lei 11.438/2006
Incentivos Fiscais → Desporto
👧 FDCA
Lei 8.069/1990, Art. 260 II
Doações Efetuadas → Código 40
👴 FDI
Lei 12.213/2010
Doações Efetuadas → Código 41
♻️ Reciclagem
Lei 14.260/2021
Incentivos Fiscais → Reciclagem
🏥 PRONON
Lei 12.715/2012, Art. 1º
1%*
Incentivos Fiscais → PRONON
💊 PRONAS
Lei 12.715/2012, Art. 3º
1%*
Incentivos Fiscais → PRONAS
* PRONON e PRONAS compartilham limite conjunto de 1% do IR Devido. Nunca some percentuais de modalidades diferentes para apresentar ao cliente — cada uma tem base legal própria e limite independente. Clique em qualquer linha para ver a ficha completa.
DÚVIDAS DE COMPLIANCE
### Perguntas que o contador precisa responder
As 5 dúvidas mais frequentes de quem valida tudo antes de assinar.
A RFB pode questionar a destinação mesmo dentro do limite legal?
Não, se três condições forem atendidas: (1) o valor destinado respeita o limite percentual da modalidade, (2) o código e a ficha DIRPF foram preenchidos corretamente, e (3) o comprovante original está disponível (recibo + comprovante bancário). A RFB pode solicitar documentação em caso de intimação, mas não pode glosar a dedução se tudo estiver em ordem. Guarde toda documentação por mínimo 5 anos.
Qual a diferença entre "abate do IR" e "aumenta a restituição"?
O valor destinado abate diretamente do IR Devido . O efeito financeiro depende da situação de cada contribuinte:
- Se IR retido na fonte IR Devido: o abate aumenta a restituição a receber
- Se IR retido IR Devido: o abate reduz o valor a pagar na declaração
Em ambos os casos, o resultado líquido para o contribuinte é zero — ele não desembolsa mais do que pagaria sem a destinação. Há um desembolso prévio (PIX/TED ao fundo) antes da declaração, mas o custo líquido final é zero .
⚠️ Nunca afirme "aumenta a restituição" como certeza — depende do saldo de IR retido de cada cliente específico.
Posso orientar a destinação após o encerramento da DIRPF?
Sim — e é o momento ideal para começar. A destinação ocorre durante o ano-calendário (1° jan a 31 dez), não na declaração. Após o encerramento da DIRPF (maio de cada ano), o posicionamento correto é:
"A destinação de 2027 começa agora. Vamos planejar juntos."
O cliente tem o ano inteiro para fazer a destinação — e pode fazê-la em parcelas ao longo do ano. Isso é planejamento tributário preventivo.
O que o cliente deve guardar como comprovação?
Para cada destinação, guardar por mínimo 5 anos :
- Comprovante bancário da transferência (PIX/TED) — com data, CNPJ destino e valor
- Recibo emitido pelo beneficiário — com todos os dados exigidos por modalidade
- Rouanet: número do PRONAC visível no recibo
- FDCA/FDI: CNPJ do fundo e número do recibo
- PRONON/PRONAS: número CNES da entidade habilitada
- Reciclagem: CNPJ da cooperativa com habilitação MAPA ativa
O Recibo de Mecenato gerado pela plataforma IncentivaBR já contém automaticamente todos os dados exigidos por modalidade.
Posso orientar o mesmo cliente em mais de uma modalidade?
Sim — cada modalidade tem base legal própria e limites independentes entre si (exceto PRONON + PRONAS que compartilham 1%). Um cliente pode destinar simultaneamente para Rouanet (6%), Esporte (7%), FDCA (6%) e FDI (6%) — cada um com seu próprio teto legal.
O único limite absoluto: a soma total destinada não pode superar o IR Devido total do contribuinte. Nunca apresente como "você pode destinar até X% do IR" — isso induz erro. Cada modalidade tem percentual específico independente.
Use o Validador Anti-Malha Fina para verificar os limites de cada cliente de forma segura.
FERRAMENTAS RELACIONADAS
### Complete o atendimento ao cliente com
Validador Anti-Malha Fina
Verifica se o cliente está dentro dos limites em todas as 7 modalidades antes de declarar
Espaço do Contador
Calculadora de limites por cliente, modelos de comprovante e FAQ profissional
Agenda Fiscal
Prazos e janelas de destinação por modalidade para planejar com antecedência

---

# GUIA DO IMPOSTO DE RENDA PARA O SERVIDOR
<!-- fonte: frontend/guia-ir-servidor.html -->

Início
Como funciona
Guia IR para Servidores
Guia exclusivo para servidores públicos
## Como declarar a destinação
Lei Rouanet no seu IR
Onde encontrar o IR Devido e como lançar corretamente na declaração anual — sem complicação, sem surpresas.
#### Nesta página
- Parte 1 — Como encontrar o valor do IR Devido
- Parte 2 — Como lançar a destinação na declaração de IR
- Parte 3 — Documentos que você precisa guardar
- Parte 4 — Exemplo numérico completo
Requisito: a destinação via Lei Rouanet exige a Declaração Completa (não a simplificada).
Servidores públicos com renda acima de 2 salários mínimos devem optar pelo modelo completo para aproveitar as deduções.
### Parte 1 — Como encontrar o valor do IR Devido
O que você destina pode voltar pra você
Imposto retido na fonte (ano) R$ 10.000
Destinação Rouanet (6%) − R$ 600
Restituição extra + R$ 600
Custo líquido pro seu bolso R$ 0
O projeto ganha. Você não perde nada. Quem "paga" é o governo — não você.
O IR Devido é o imposto calculado sobre sua renda, antes de abater o que já foi retido na fonte.
Você precisa desse número para calcular o limite de destinação (6% do IR Devido). Há três formas de encontrá-lo:
#### Via declaração anterior / e-CAC (mais fácil)
Recomendado
Se você faz declaração anual, o IR Devido do ano anterior é uma boa estimativa para o ano atual.
Acesse pelo programa IRPF ou pelo e-CAC (portal da Receita Federal).
- 1
##### Acesse o e-CAC
Entre em cav.receita.fazenda.gov.br com sua conta gov.br
(CPF + senha ou certificado digital). Servidores federais já têm conta gov.br.
- 2
##### Vá em "Declarações e Demonstrativos"
No menu lateral, clique em
Declarações e Demonstrativos
IRPF — Declaração de Ajuste Anual
- 3
##### Selecione o exercício mais recente
Clique na declaração do último ano entregue e abra o Resumo da Declaração.
- 4
##### Localize o campo "Imposto Devido"
Na seção Cálculo do Imposto, procure a linha:
Resumo da Declaração — Cálculo do Imposto
Rendimentos Tributáveis: R$ 85.200,00
Deduções: R$ 15.600,00
Base de Cálculo: R$ 69.600,00
Imposto Devido: R$ 8.200,00
IR Retido na Fonte: R$ 9.500,00
Imposto a Restituir: R$ 1.300,00
O valor que você precisa é o Imposto Devido — no exemplo: R$ 8.200,00.
Seu limite de destinação = R$ 8.200 × 6% = R$ 492,00.
Dica: mesmo que o Imposto a Restituir seja diferente de zero, o que importa para o limite é o
Imposto Devido , não o valor retido ou restituído.
#### Via imposto retido na fonte (quando não tem a declaração em mãos)
Estimativa
Se você não tem a declaração IRPF em mãos, pode usar o imposto retido na fonte ao longo do ano como estimativa.
Esse valor aparece nos seus contracheques mensais e é uma aproximação razoável — mas o valor exato está na declaração.
- 1
##### Abra o contracheque de dezembro do ano anterior
O contracheque de dezembro geralmente acumula o total anual na coluna "Ano" ou "Acumulado".
- 2
##### Localize "IRRF" na coluna de descontos
Contracheque — Dezembro
Provento Bruto: R$ 7.100,00
INSS: R$ 710,00
IRRF (mês): R$ 680,00
IRRF (acumulado ano): R$ 7.900,00
- 3
##### Use o IRRF acumulado como estimativa do IR Devido
No exemplo: IRRF acumulado = R$ 7.900 → limite estimado = R$ 7.900 × 6% = R$ 474,00.
Atenção: o IRRF retido é uma estimativa do IR Devido. O valor exato só é calculado na declaração anual,
considerando todas as deduções (saúde, dependentes, educação etc.). Use o
MeuLimite
para uma estimativa mais precisa.
#### Via SIGEPE (servidor federal)
Federal
O SIGEPE (Sistema de Gestão de Pessoas do Governo Federal) permite consultar o histórico de remuneração
e os valores de IRRF retidos na fonte por competência.
- 1
##### Acesse o SIGEPE
Entre em sigepe.gov.br com seu login gov.br (CPF + senha).
- 2
##### Vá em "Contracheque" → "Histórico de Pagamentos"
Selecione o ano de referência e exporte o resumo anual.
- 3
##### Some todos os valores de "IRRF" do ano
A soma dos IRRF mensais é o total retido na fonte — use como base de estimativa do IR Devido.
O SIGEPE também emite o Informe de Rendimentos anual, que é a fonte mais confiável para preencher
a declaração. Disponível em: SIGEPE → Minha Área → Informe de Rendimentos .
### Parte 2 — Como lançar a destinação na declaração de IR
Após fazer a transferência para o projeto cultural, você precisa registrar a destinação na sua declaração anual do IRPF
(programa da Receita Federal). Siga os passos abaixo.
- 1
##### Abra o programa IRPF da Receita Federal
Baixe a versão mais recente em gov.br/receitafederal ou use o app Meu Imposto de Renda
no celular. Abra sua declaração do ano correspondente.
- 2
##### Acesse a ficha "Doações Efetuadas"
No menu lateral do programa, localize a ficha:
Doações Efetuadas
Esta ficha reúne todas as doações dedutíveis do IR — inclusive as da Lei Rouanet.
- 3
##### Clique em "Novo" e selecione o Código 41
Na janela que abre, preencha os campos:
Ficha: Doações Efetuadas — Nova Doação
Código da doação
41 — Atividade Cultural e Artística
CNPJ do beneficiário
[CNPJ do proponente cultural]
Nome do beneficiário
[Nome do proponente]
Data do pagamento
DD/MM/AAAA
Valor pago
R$ [valor destinado]
O CNPJ do proponente e o nome completo constam no Recibo de Mecenato
que a IncentivaBR emite para você após a confirmação do pagamento.
- 4
##### Confira o limite automático
O programa IRPF calcula automaticamente se você está dentro do limite de 6%.
Se ultrapassar, ele avisa antes de você enviar a declaração.
- 5
##### Verifique a restituição (ou imposto a pagar)
Após lançar a doação, observe o Resumo da Declaração. O valor da restituição deve aumentar
exatamente no valor que você doou — comprovando que a destinação foi reconhecida.
Fiz a transferência parcelada (várias vezes no ano). Lance cada transferência individualmente na ficha
"Doações Efetuadas" com a data e valor correspondentes. Ou lance o total anual em uma única linha com a data da última
transferência — desde que o total não ultrapasse 6% do IR Devido.
### Parte 3 — Documentos que você precisa guardar
A Receita Federal pode solicitar comprovação em até 5 anos. Guarde os seguintes documentos:
Documento
Onde obter
Prazo
Recibo de Mecenato
(emitido pela IncentivaBR)
Dashboard do site · E-mail de confirmação
5 anos
Comprovante de transferência
(extrato / PIX)
App ou internet banking do seu banco
5 anos
CNPJ e dados do proponente
Recibo de Mecenato · Página do projeto no SALIC
5 anos
Zero risco de malha fina se você respeitar o limite de 6%, informar o código correto (41) e
guardar os comprovantes. Milhões de brasileiros destinam via Lei Rouanet todo ano — é um procedimento reconhecido
e incentivado pela própria Receita Federal.
Tem contador? Peça para ele confirmar o valor do IR Devido e validar a destinação antes de fazer
a transferência. A anuência do contador é a forma mais segura — especialmente se você tem deduções complexas
(dependentes, saúde, previdência privada etc.).
### Parte 4 — Exemplo numérico completo
Veja como o fluxo funciona do início ao fim para um servidor público típico:
##### Perfil: Analista Judiciário Federal
Salário bruto mensal
R$ 7.100,00
Rendimento tributável anual
R$ 85.200,00
Deduções (INSS + dependentes + saúde)
R$ 17.000,00
Base de cálculo
R$ 68.200,00
IR Devido (alíquota efetiva ~12%)
R$ 8.184,00
Limite Lei Rouanet (6% do IR Devido)
R$ 491,00
IR retido na fonte (anual)
R$ 9.500,00
Restituição sem destinação
R$ 1.316,00
Restituição com destinação de R$ 491
R$ 1.807,00
O servidor destinou R$ 491 para cultura e recebeu R$ 491 a mais de restituição.
Custo líquido real: R$ 0,00. O dinheiro que iria para o governo foi para a cultura popular brasileira.
#### Pronto para destinar seu IR?
Use o MeuLimite para descobrir seu limite exato e, se quiser, confirme com seu contador antes de fazer a transferência.
Calcular com MeuLimite
Destinar agora
Fonte oficial:
Manual da Declaração — Receita Federal
IncentivaBR programa de computador registrado no INPI sob o n BR512025000647-0.
Marca mista depositada no INPI sob os processos n 939403005 (NCL 35) e n 939403110 (NCL 42).
Documenta o t cnica registrada no 2 Of cio de Registro Civil, T tulos e Documentos e Pessoas Jur dicas de Bras lia/DF.
2026 Adacto Artur Dornas de Oliveira. Todos os direitos reservados. vedada a reprodu o, distribui o ou engenharia reversa, total ou parcial, sem autoriza o expressa.

---

# COMO FUNCIONA A DESTINACAO
<!-- fonte: frontend/como-funciona.html -->

Início
Como funciona
Projeto
Destinação
Guia completo · Lei 8.313/1991
## Como funciona a Lei Rouanet?
Destine até 6% do seu IR para projetos culturais — sem custo adicional. Em 4 passos simples.
Requisito: declaração completa (modelo completo) do Imposto de Renda
### O que é a Lei Rouanet?
A Lei 8.313/1991 , conhecida como Lei Rouanet, permite que pessoas físicas
destinem até 6% do seu Imposto de Renda devido para projetos culturais
aprovados pelo Ministério da Cultura (MinC) e cadastrados no sistema SALIC .
A sacada principal
O valor destinado é deduzido integralmente do imposto — você redireciona dinheiro que já iria para o governo para projetos culturais que você acredita.
Custo adicional: zero.
### 4 passos para destinar
Do cálculo ao comprovante — menos de 30 minutos.
PASSO 1
#### Calcule seu limite
Use o MeuLimite para descobrir quanto pode destinar com base no seu rendimento anual e IR devido. Limite máximo: 6% do IR devido.
Exemplo: IR devido de R$ 10.000 → você pode destinar até R$ 600 para projetos culturais.
PASSO 2
#### Escolha o projeto
Acesse nosso projeto aprovado pelo MinC/SALIC. Você verá o PRONAC, o proponente e quanto ainda pode ser captado.
PASSO 3
#### Transfira para o FNC
A destinação é feita via transferência bancária para o Fundo Nacional de Cultura (FNC) , administrado pelo Banco do Brasil.
Banco: Banco do Brasil (001)
Agência: 1419-2
Conta: 36.068-6
Favorecido: Fundo Nacional de Cultura
No campo de referência, informe o PRONAC do projeto (fornecido na confirmação).
PASSO 4
#### Declare no IR
Na declaração anual (DIRPF), informe o valor destinado em:
Ficha: Doações Efetuadas
Código: 41 — Estatuto da Criança e do Adolescente / Lei Rouanet
Guarde o comprovante de transferência bancária — ele é exigido pela Receita Federal.
### Por que 99% não destinam
De cada 100 contribuintes elegíveis, 99 nunca destinaram um centavo . Não é falta de vontade — é barreira com solução.
"Parece muito complicado"
Tem solução
É menos complicado do que parece. São 4 passos: calcular → escolher projeto → fazer TED → declarar. O IncentivaBR guia cada etapa.
"Desconfio que o dinheiro não chega"
Tem solução
Os projetos são aprovados pelo Ministério da Cultura e listados no SALIC (sistema oficial do governo federal). O FNC é gerido pelo Banco do Brasil . Você pode verificar qualquer dado no site oficial antes de transferir.
"O valor é pequeno, não compensa"
Tem solução
Não é dinheiro novo — é o imposto que você já pagaria ao governo , só redirecionado. Seu bolso fica exatamente igual. E mesmo R$ 300 pagam 10 aulas de música para jovens em situação de vulnerabilidade.
"Tenho medo de cair na malha fina"
Tem solução
Não há risco se você respeitar o limite de 6% e guardar o comprovante. Milhões de pessoas fazem isso todo ano sem problema. O IncentivaBR bloqueia qualquer valor acima do limite automaticamente.
"Deixo para o ano que vem"
Tem solução
O prazo é 31 de dezembro do ano-base . Quem deixa para depois perde o benefício daquele ano. 5 anos de procrastinação = mais de R$ 5.000 que poderiam ter ido para a cultura.
Para servidores públicos
### Você tem condição ideal para destinar
Servidores têm salário fixo e IR previsível — condição ideal para planejar a destinação com antecedência. O imposto retido em folha não impede a destinação. O que importa é o IR devido calculado na declaração anual completa.
Estimativa rápida
IR retido no ano passado × 6% = valor máximo que você pode destinar este ano.
Exemplo: reteve R$ 18.000 → pode destinar até R$ 1.080 para projetos culturais.
Não existe procedimento diferenciado para servidores — as mesmas regras da Lei Rouanet se aplicam a todos os contribuintes pessoa física.
### Dúvidas frequentes
Quem pode destinar?
Qualquer contribuinte pessoa física que faça a declaração completa do IR e tenha IR devido (não isento).
Preciso ter IR a pagar?
Não necessariamente — quem tem restituição a receber também pode destinar, desde que tenha IR devido calculado. A destinação reduz o IR devido, o que pode aumentar a restituição .
Qual o prazo?
A transferência deve ser feita dentro do exercício fiscal (1º de janeiro a 31 de dezembro do ano-base). Pode ser feita a qualquer momento durante o ano.
O que acontece se destinar mais que 6%?
O excedente não é deduzível — você perderá o benefício fiscal sobre o valor acima do limite. Use o MeuLimite para saber seu limite exato.
É seguro?
Sim. Os projetos são aprovados pelo MinC e listados no SALIC (sistema oficial do governo federal). O FNC é gerido pelo Banco do Brasil.
### O projeto fala com você
O Orquestra das Periferias do DF é cultura popular que ressoa em cada área do serviço público.
#### Justiça e Judiciário
O Orquestra das Periferias do DF leva cultura popular brasileira a praças, teatros e centros culturais. Você pode fazer isso acontecer.
#### Educação
O Orquestra das Periferias do DF preserva e difunde a cultura popular brasileira — amplia o que você faz na sala de aula.
🛡️
#### Segurança Pública
Cultura fortalece comunidades. O Orquestra das Periferias do DF leva arte e identidade cultural a quem mais precisa.
#### Saúde
Arte e música têm impacto comprovado na saúde mental. O Orquestra das Periferias do DF é saúde pública pela via cultural.
#### Fazenda e Finanças
É o mesmo imposto que você já pagaria ao governo — só redirecionado. Você sabe disso melhor do que ninguém.
#### Cultura e Artes
Apoiar o Orquestra das Periferias do DF fortalece o ecossistema cultural brasileiro e abre portas para o próximo grande nome da música popular.
### Pronto para destinar?
Comece pelo MeuLimite — descubra exatamente quanto pode direcionar.
Calcular meu limite
Ver o Orquestra das Periferias do DF
Guia do Servidor →
IncentivaBR programa de computador registrado no INPI sob o n BR512025000647-0.
Marca mista depositada no INPI sob os processos n 939403005 (NCL 35) e n 939403110 (NCL 42).
Documenta o t cnica registrada no 2 Of cio de Registro Civil, T tulos e Documentos e Pessoas Jur dicas de Bras lia/DF.
2026 Adacto Artur Dornas de Oliveira. Todos os direitos reservados. vedada a reprodu o, distribui o ou engenharia reversa, total ou parcial, sem autoriza o expressa.

---

# PASSO A PASSO DA DESTINACAO
<!-- fonte: frontend/passo-a-passo.html -->

Início
Como funciona
Passo a passo
6 passos · menos de 10 minutos
## Passo a passo completo
Do cálculo ao comprovante — veja exatamente o que fazer em cada etapa para destinar seu IR com segurança.
PASSO 1
~2 minutos
### Calcule seu limite com o MeuLimite
Antes de tudo, descubra quanto do seu IR você pode destinar. O limite é 6% do IR Devido — não do salário. Use o MeuLimite para calcular com base no seu rendimento anual.
Não sabe o IR Devido? O contracheque de dezembro mostra o IRRF acumulado — é uma boa estimativa. Veja o Guia do Servidor .
Abrir o MeuLimite
PASSO 2
~2 minutos
### Conheça o Orquestra das Periferias do DF
Acesse a página do projeto para ver o PRONAC, o proponente, a meta de captação e o impacto cultural. Você verifica tudo no SALIC (sistema oficial do governo federal) antes de destinar.
Ver o Orquestra das Periferias do DF
PASSO 3
~3 minutos
### Registre sua destinação no sistema
Clique em "Destinar para este projeto", faça login (ou crie sua conta em segundos) e preencha o valor respeitando o limite de 6%.
Login ou cadastro rápido
Informe o valor
Confirme seus dados
Receba os dados do FNC
Iniciar destinação
PASSO 4
~2 minutos
### Faça a transferência para o FNC
Transfira o valor para a conta oficial do Fundo Nacional de Cultura (FNC) no Banco do Brasil usando PIX, TED ou transferência.
Banco: Banco do Brasil (001)
Agência: 1419-2
Conta: 36.068-6
Favorecido: Fundo Nacional de Cultura
No campo descrição/referência , informe o PRONAC do projeto . Guarde o comprovante — print do app do banco funciona!
PASSO 5
~1 minuto
### Envie o comprovante
Após a transferência, volte ao sistema e envie o comprovante bancário (PDF, JPG ou PNG, até 5MB) para finalizar o registro.
Pronto! Você recebe um número de protocolo e um comprovante pessoal. O Recibo de Mecenato oficial é emitido pelo proponente em até 60 dias.
PASSO 6 — FINAL
### Declare no IR — Código 41
Na declaração anual (DIRPF), informe a destinação na ficha Doações Efetuadas :
Ficha: Doações Efetuadas
Código: 41 — Atividade Cultural e Artística (Lei Rouanet)
Valor: o montante transferido
Guarde o comprovante da transferência e o Recibo de Mecenato por no mínimo 5 anos.
Ver guia completo para servidores →
### Todo o processo leva menos de 10 minutos.
Seu IR vira música, arte e cultura para jovens que precisam.
Começar pelo MeuLimite
Tirar dúvidas →
IncentivaBR programa de computador registrado no INPI sob o n BR512025000647-0.
Marca mista depositada no INPI sob os processos n 939403005 (NCL 35) e n 939403110 (NCL 42).
Documenta o t cnica registrada no 2 Of cio de Registro Civil, T tulos e Documentos e Pessoas Jur dicas de Bras lia/DF.
2026 Adacto Artur Dornas de Oliveira. Todos os direitos reservados. vedada a reprodu o, distribui o ou engenharia reversa, total ou parcial, sem autoriza o expressa.

---

# PERGUNTAS FREQUENTES
<!-- fonte: frontend/faq.html -->

Perguntas Frequentes
## Tudo sobre Lei Rouanet
Respostas claras para destinar seu IR com segurança e confiança.
Todas
Lei Rouanet
Servidores Públicos
Processo
Declaração IR
#### Nenhuma pergunta encontrada
Tente buscar com outras palavras.
### Lei Rouanet
Vou perder dinheiro ou pagar mais imposto?
Não! O valor destinado é deduzido integralmente do IR devido. Você redireciona dinheiro que já iria para o governo para um projeto cultural. Custo adicional: zero .
Exemplo: IR devido R$ 10.000 → destina R$ 600 (6%) → paga só R$ 9.400 ao governo. A diferença vai para a cultura.
O que é a Lei Rouanet?
A Lei 8.313/1991 permite que pessoas físicas destinem até 6% do IR devido para projetos culturais aprovados pelo Ministério da Cultura (MinC) e cadastrados no sistema SALIC .
O dinheiro vai para o Fundo Nacional de Cultura (FNC) , gerido pelo Banco do Brasil, e é repassado ao projeto escolhido.
É seguro? Como verifico os dados?
É verificável — e essa é a melhor garantia. Os projetos são aprovados e publicados no SALIC (salic.cultura.gov.br), sistema oficial do governo federal. O FNC é gerido pelo Banco do Brasil.
Você pode verificar qualquer projeto em salic.cultura.gov.br pelo número PRONAC.
Qual o limite que posso destinar?
O limite é 6% do IR devido — não do salário, mas do imposto calculado. Use nossa calculadora para saber o valor exato.
Destinar acima do limite não gera benefício fiscal — o excedente não é dedutível.
### Servidores Públicos
Tenho imposto retido em folha. Posso destinar assim mesmo?
Sim! O imposto retido na fonte não impede a destinação. O que importa é o IR devido calculado na sua declaração anual completa.
Se você tem restituição a receber , a destinação aumenta esse valor. Se tem imposto a pagar , a destinação reduz o saldo. Em ambos os casos, seu bolso fica igual.
Existe algum procedimento específico para servidores públicos?
Não. Servidores seguem as mesmas regras das demais pessoas físicas.
A única diferença — favorável — é que você tem previsibilidade de renda (salário fixo), o que facilita o planejamento da destinação.
Como estimo quanto posso destinar com base no meu contracheque?
O valor exato só é conhecido após simular a declaração completa, mas você pode estimar assim:
IR retido no ano anterior ≈ IR devido neste ano
Limite Rouanet (6%) = IR retido em 2024 × 0,06
Exemplo: reteve R$ 20.000 → pode destinar até R$ 1.200
Use nossa Calculadora IR → para uma estimativa mais precisa.
Existe risco de cair na malha fina?
Zero risco se você respeitar o limite de 6%, informar o valor correto na ficha "Doações Efetuadas" (código 41) e guardar o comprovante por 5 anos.
Milhões de brasileiros destinam via Lei Rouanet todo ano sem problema. A destinação é totalmente legal e reconhecida pela Receita Federal.
Posso destinar para projetos fora da minha cidade?
Sim. Você pode escolher projetos de qualquer localidade do Brasil , independentemente do seu domicílio fiscal. A Lei Rouanet não restringe por região.
### Processo
Como faço a transferência? Posso usar PIX?
Sim! Você pode usar PIX, TED ou transferência bancária para a conta do FNC no Banco do Brasil:
- Banco: Banco do Brasil (001)
- Agência: 1419-2
- Conta: 36.068-6
- Favorecido: Fundo Nacional de Cultura
No campo descrição/referência, informe o PRONAC do projeto (fornecido no sistema após registrar a destinação).
Que comprovante preciso enviar?
Envie o comprovante da transferência em PDF, JPG ou PNG (até 5MB). Deve mostrar data, valor e destinatário (FNC).
Print da tela do aplicativo do banco funciona perfeitamente!
O que é o Comunicado de Mecenato?
O Comunicado de Mecenato é o recibo oficial emitido pelo proponente do projeto. É o documento com valor fiscal para a declaração do IR — diferente do comprovante da plataforma (só para controle pessoal).
- O proponente tem prazo legal de 15 dias para emitir
- Deve conter: seu nome, CPF, valor, data e dados do projeto
- Guarde por no mínimo 5 anos
Quais documentos preciso guardar e por quanto tempo?
Guarde por mínimo 5 anos :
- Comprovante de transferência bancária
- Comunicado de Mecenato emitido pelo proponente
- Registro da destinação na plataforma IncentivaBR
Dica: crie uma pasta "Destinações IR 2025" no Google Drive ou OneDrive com backup automático.
### Declaração do IR
Até quando posso fazer a destinação?
A transferência deve ser feita dentro do exercício fiscal — entre 1 de janeiro e 31 de dezembro do ano-base.
Para entrar na declaração de 2026 (ano-base 2025), o depósito deve ter sido feito até 31 de dezembro de 2025 .
Como declaro a destinação no IR?
Na DIRPF, informe em:
- Ficha: Doações Efetuadas
- Código: 41 — Lei Rouanet / Atividade Cultural e Artística
Guarde o comprovante da transferência bancária — a Receita Federal pode solicitar como prova.
Ver guia completo: IR Devido + Ficha Doações →
Tenho restituição a receber. A destinação afeta esse valor?
Sim — para melhor! A destinação aumenta o valor da sua restituição.
IR retido: R$ 24.000 · IR devido: R$ 22.000 · Restituição original: R$ 2.000
Com destinação de R$ 1.500 → novo IR: R$ 20.500 → nova restituição: R$ 3.500 ✅
Você destinou R$ 1.500 para a cultura e ainda recebe R$ 1.500 a mais de restituição. Custo real: zero .
Posso destinar para mais de um projeto cultural?
Sim. Você pode distribuir seu limite entre quantos projetos SALIC quiser, desde que a soma não ultrapasse 6% do IR devido.
Exemplo: IR devido R$ 20.000 → limite R$ 1.200 → R$ 700 para o Orquestra das Periferias do DF + R$ 500 para outro projeto = R$ 1.200 ✅
Sou isento de IR. Posso destinar?
Infelizmente não. A destinação é um percentual do IR devido. Quem é isento tem IR devido = R$ 0, então não há base para a dedução.
Use nossa Calculadora IR → para verificar se você tem IR devido.
#### Ainda tem dúvidas?
Fale conosco pelo WhatsApp ou use a calculadora para ver seu limite.
WhatsApp
Calculadora IR
Guia do Servidor
Destinar agora
IncentivaBR programa de computador registrado no INPI sob o n BR512025000647-0.
Marca mista depositada no INPI sob os processos n 939403005 (NCL 35) e n 939403110 (NCL 42).
Documenta o t cnica registrada no 2 Of cio de Registro Civil, T tulos e Documentos e Pessoas Jur dicas de Bras lia/DF.
2026 Adacto Artur Dornas de Oliveira. Todos os direitos reservados. vedada a reprodu o, distribui o ou engenharia reversa, total ou parcial, sem autoriza o expressa.