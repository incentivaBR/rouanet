...
test.describe('Projetos Rouanet — página do Circuito do Forró', () => {
  ...
  test('página carrega com título reconhecível', async ({ page }) => {
    await expect(page).toHaveTitle(/Forró|DestineAI|Projeto/i);
  });
  ...
  test('botão CTA "Destinar agora" aponta para wizard com PRONAC 252026', async ({ page }) => {
    const btn = page.locator('#btnDestinar');
    await expect(btn).toBeVisible({ timeout: 5_000 });

    const href = await btn.getAttribute('href');
    expect(href).toContain('destinar-rouanet.html');
    expect(href).toContain('pronac=252026');
  });
...
test.describe('Projeto detalhes — projeto-detalhes.html', () => {
  test('carrega com PRONAC 252026 via query param', async ({ page }) => {
    await page.goto('/projeto-detalhes.html?pronac=252026');
    await expect(page.locator('body')).toHaveClass(/dai-ready/, { timeout: 8_000 });
